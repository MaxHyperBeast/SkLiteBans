...
public class ExprEntryID extends SimpleExpression<Long> {
    static {
        Skript.registerExpression(ExprEntryID.class, Long.class, ExpressionType.SIMPLE, "id of %litebansentry%", "%litebansentry%'s id");
    }
    private Expression<Entry> entry;
    @Override
    protected Long @Nullable [] get(Event event) {
        if (event instanceof EvtEntryAdded.EntryAddedEvent) {
            return new Long[] {((EvtEntryAdded.EntryAddedEvent) event).getEntry().getId()};
        }
        return null;
    }
