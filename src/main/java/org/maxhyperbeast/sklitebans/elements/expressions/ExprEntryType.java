...
public class ExprEntryType extends SimpleExpression<String> {
    static {
        Skript.registerExpression(ExprEntryType.class, String.class, ExpressionType.SIMPLE, "type of %litebansentry%", "%litebansentry%'s type");
    }
    private Expression<Entry> entry;
    @Override
    protected String @Nullable [] get(Event event) {
        if (event instanceof EvtEntryAdded.EntryAddedEvent) {
            return new String[] {((EvtEntryAdded.EntryAddedEvent) event).getEntry().getType()};
        }
        return null;
    }
